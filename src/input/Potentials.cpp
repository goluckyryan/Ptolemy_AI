...
bool LiPotential(int A, int Z, double E){
  int N   = A-Z;
  double A3 = pow(A, 1./3.);
  v  = 179.5 - 0.346 * E;
  r0 = 1.13;
...
bool SuHanPotential(int A, int Z, double E){
  int N   = A-Z;
  double A3 = pow(A, 1./3.);
  double rc = 1.24 * A3 + 0.12;
  double EC = 1.73 * Z * 2 / rc;
  v  = 108.6 - 0.18 * (E - EC);
  r0 = 1.36;
  a  = 0.67;
  vi  = 13.5/(1 + exp((60 - E + EC)/20.) );
  ri0 = 1.45;
  ai  = 0.58;
  vsi  = 10.5/(1+exp((E-EC - 60.)/20.));
  rsi0 = 1.45;
  asi  = 0.58;
  vso  = 0;
  rso0 = 0;
...
bool BassaniPotential(int A, int Z, double E){
  int N   = A-Z;
  double A3 = pow(A, 1./3.);
  v  = 177.3;
  r0 = 1.337;
...
void CallPotential(std::string name, int A, int Z, double E, int Zproj){
  ...
  if( name == "A" ) AnCaiPotential(A, Z, E);
  if( name == "H" ) HSSPotential(A, Z, E);
  if( name == "B" ) BojowaldPotential(A, Z, E);
  if( name == "D" ) DaehnickPotential(A, Z, E);
  //if( name == "C" ) DaehnickPotential(A, Z, E);
  if( name == "L" ) LohrPotential(A, Z, E);
  if( name == "Q" ) PereyPereyPotential(A, Z, E);
  
  if( name == "Z" ) ZhangPangLouPotential(A, Z, E, Zproj);
  
  if( name == "K" ) KoningPotential(A, Z, E, Zproj);
  if( name == "V" ) VarnerPotential(A, Z, E);
  if( name == "M" ) MenetPotential(A, Z, E);
  if( name == "G" ) BecchettiPotential(A, Z, E);
  if( name == "P" ) PereyPotential(A, Z, E);

  if( name == "x" ) XuPotential(A, Z, E);
  if( name == "l" ) LiangPotential(A, Z, E);
  if( name == "p" ) PangPotential(A, Z, E, Zproj);
  if( name == "c" ) LiPotential(A, Z, E);
  if( name == "t" ) TrostPotential(A, Z, E);
  if( name == "h" ) HyakutakePotential(A, Z, E);
  if( name == "b" ) BecchettiPotential(A, Z, E);
  
  if( name == "s" ) SuHanPotential(A, Z, E);
  if( name == "a" ) AvrigeanuPotential(A, Z, E);
  if( name == "f" ) BassaniPotential(A, Z, E);
  
  if( name == "Y" ) CustomYPotential(A, Z, E);
  if( name == "X" ) CustomXPotential(A, Z, E);
