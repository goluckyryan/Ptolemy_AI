...
int main() {
    // ===== System: 16O(p,p')16O*(3-, Ex=6.13 MeV) Ep=10 MeV =====
    int Ap = 1, Zp = 1, At = 16, Zt = 8;
    double Elab = 10.0;
    double Ex = 6.13;
    int LX = 3;
    // B(E3) = 8.2 e²·fm^6 for 16O 3- state
    double BELX = 8.2;  // e²·fm^6 (barn^3 = fm^6/10^6 → use fm units)
    int Lmax = 35;  // LIN/LOUT range

    // OM parameters - incoming (p+16O at 10 MeV, Perey optical)
    double V_in=48.0, r0_in=1.17, a_in=0.75;
    double Vi_in=3.5, ri0_in=1.32, ai_in=0.6;
    double Vsi_in=9.0, rsi0_in=1.32, asi_in=0.6;
    double rc0_in=1.25;

    // OM parameters - outgoing (same potential, p+16O* at 3.28 MeV CM)
    double V_out=48.0, r0_out=1.17, a_out=0.75;
    double Vi_out=3.5, ri0_out=1.32, ai_out=0.6;
    double Vsi_out=9.0, rsi0_out=1.32, asi_out=0.6;
    double rc0_out=1.25;
    ...
    dwba.Incoming.Pot = {V_in, r0_in, a_in,
                         ...
                         rc0_in};
    // Ptolemy step size: h = min(2π/k, 1.0) / STPSPR (default STPSPR=10)
    // Fortran default: STEPSZ=0.10 fm = min(2π/k,1.0)/10
    // No PARAMETERSET for p+16O, so use default STPSPR=10
    int STEPSPER = 10;  // Fortran default (STEPSZ=0.10 fm)
    ...
    for (int LI = 0; LI <= Lmax; LI++) {
        ...
        for (int LO = LO_min; LO <= LO_max; LO += 2) {
            ...
            for (int i = 0; i < NUMPT; i++) {
                ...
                // chi diagnostic for LI=2,LO=1 at ALL Gauss points
                if (LI==2 && LO==1) {
                    double H_nuc = H_r_wt[i]; double H_coul = H_c_wt[i];
                    double DWR = chi_out_val.real()*chi_in_val.real() - chi_out_val.imag()*chi_in_val.imag();
                    double DWI = chi_out_val.real()*chi_in_val.imag() + chi_out_val.imag()*chi_in_val.real();
                    double termR = H_nuc*DWR - H_i_wt[i]*DWI + H_coul*DWR;
                    double termI = H_nuc*DWI + H_i_wt[i]*DWR + H_coul*DWI;
                    std::cerr << std::scientific << std::setprecision(6)
                              << "CHI_PT " << i << " r=" << gl_pts[i]
                              << " ci=(" << chi_in_val.real() << "," << chi_in_val.imag() << ")"
                              << " co=(" << chi_out_val.real() << "," << chi_out_val.imag() << ")"
                              << " Hn=" << H_nuc << " Hc=" << H_coul
                              << " TR=" << termR << " TI=" << termI
                              << std::endl;
                }
            ...
            int LDEL = LO - LI;
            int Lmax_elastic_est = 0;   // IRTOIN disabled for light systems (p+16O, eta~0.4)
